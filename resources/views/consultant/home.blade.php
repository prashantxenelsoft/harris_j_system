    ...
    <section class="login-as-consultant">
        <div class="container">
            ...
            <div class="row pb-5">
                    ...
                    <div class="consultant-dashboard-left-col">
                        ...
                        <div class="claim-summary-dashboard-consultant">
                            ...
                            @php
                                $firstClaim = $dataClaims->first();
                                $record = $firstClaim ? json_decode($firstClaim->record, true) : null;
                                $claimNo = $record['claim_no'] ?? '';
                                $filteredClaims = $dataClaims->filter(function ($claim) use ($claimNo) {
                                    $rec = json_decode($claim->record, true);
                                    return isset($rec['claim_no']) && $rec['claim_no'] === $claimNo;
                                });
                            @endphp
                            ...
                            <div class="claim-form-conusltant-dashboard">
                                ...
                                <div class="claim-tab-dummy-consulatnt">
                                    ...
                                    data-bs-target="#claimModal"
                                    data-applyoncell="{{ $record['applyOnCell'] ?? '' }}">
                                    <span class="text-danger fw-bold">Individual Claims ( {{ str_pad($filteredClaims->count(), 2, '0', STR_PAD_LEFT) }} )</span>
                                    </a>
               ...
               <script>
                    // 🔹 Full claim list exposed from backend
                    ...
                    document.querySelectorAll('.open-claim-modal').forEach(button => {
                        button.addEventListener('click', function () {
                            ...
                            // 🔸 Filter matching claims by applyOnCell
                            ...
                            const record = JSON.parse(firstClaim?.record || '{}');
                            const status = (firstClaim?.status || 'draft').toLowerCase();
                            const claimNo = record.claim_no || 'N/A';
                            const claimCount = claims.length;

                            // 🔹 Update modal header
                            document.querySelector('.model_c_form span').textContent = claimNo;
                            document.querySelector('.claim_hour_title .text-danger').textContent = `Individual Claims ( ${String(claimCount).padStart(2, '0')} )`;

                            // 🔹 Update icon status strips
                            updateClaimStatusIcon(status);
                            updateTimeSheetStatusIcon(status);
                                ...
                                <div class="row tabs_type_row mb-4">
                                    <div class="col-md-6 p-0">
                                        ...
                                        <div class="bottom_ic_details detial_row">
                                            ...
                                            ${r.locationFrom ? `<div class="e_location_from"><span class="fw-bold">Location From</span><span>${r.locationFrom}</span></div>` : ''}
                                            ${r.locationTo ? `<div class="e_location_to"><span class="fw-bold">Location To</span><span>${r.locationTo}</span></div>` : ''}
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-3 p-0">
                                        <div class="e_remark_box">
                                            ...
                                            <p class="mt-2"><span>${r.remarks || ''}</span></p>
                                            </div>
                                        </div>
                                    </div>
                                </div>`;
                    ...
                    // 🔹 Reusable icon updater
                    function setActiveIcon(status, selector) {
                        const icons = document.querySelectorAll(`${selector} li span`);
                        icons.forEach(span => span.classList.remove("active"));
                        ...
                        const index = statusMap[status?.toLowerCase()];
                        if (index !== undefined && icons[index]) {
                            icons[index].classList.add("active");
                        }
                    }

                    function updateClaimStatusIcon(status) {
                        setActiveIcon(status, "#claimStatus");
                    }

                    function updateTimeSheetStatusIcon(status) {
                        setActiveIcon(status, "#timeSheetStatus");
                    }
                </script>
                ...
                <div class="modal-dialog modal-dialog-centered modal-xl">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <div class="model_employee">
                                <div class="model_emp_img">
                                    <!-- Optional Employee Image -->
                                </div>
                                <div class="name_n_id">
                                    <span class="model_emp_name">{{ $consultant->emp_name ?? 'N/A' }}</span>
                                    <span class="model_emp_id">Employee Id : {{ $consultant->emp_id ?? 'N/A' }}</span>
                                </div>
                            </div>
                            ...
                            <span class="text-danger me-4 fw-bold">Individual Claims ( 00 )</span>
                            </div>
                        ...
                        <div class="modal-body">
                            ...
                            <!-- Your status icon strip (already shown above) -->
                            ...
                            <div class="container-fluid">
                                <!-- Dynamic claim rows will be injected here by JS -->
                            </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-3 col-xxl-4">
                        <div class="timesheet-overview-consultant" id="home_page_timesheet">
                            ...
                            @endif
                            <div class="timesheet-yet-submitted">
                                <p>Timesheet Overview</p>
                                <h6>Yet to be Submitted</h6>
                            </div>
                            </div>
                        </div>
                    </div>
