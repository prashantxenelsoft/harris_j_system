...
<script>
   document.addEventListener("DOMContentLoaded", function () {
      if (!localStorage.getItem("timesheetMonth") || !localStorage.getItem("timesheetYear")) {
         const now = new Date();
         localStorage.setItem("timesheetMonth", now.getMonth()); 
                     ...
                     <div class="left-col-top-bar">
                      <div class="left-col-top-bar">
                        ...
                        <div id="reporting_fileds_data" class="client-details-consultant">
                          <ul>
    <li>
        <input type="text" id="corporate_email" name="corporate_email" placeholder="Enter Corporate Email Of Consultant">
    </li>
    <li>
...
(function () {
    ...
    function updateSlider() {
        ...
        if (parseInt(modal.getAttribute("data-full-hour") || "0") === 24) {
            maxTooltip.textContent = "12:00 AM"; 
         }
    }
    ...
    document.head.appendChild(style);
                        ...
                        <script>
                          document.addEventListener("DOMContentLoaded", function () {
                           ...
                           const monthSelect = document.getElementById("monthSelect");
                           const yearSelect = document.getElementById("yearSelect");
                           const dropdownSuggestions = [ { label: "ML", icon: `<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                           ...
                           </svg>` }, { label: "AL", icon: `<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                           ...
                           </svg>` }, { label: "Custom", icon: `<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
   ...
   return;
                              ...
                              function saveWorkingHour(val) {
                                 ...
                                 if (hourVal >= 9 && hourVal <= 24) {
                                    ...
                                    modalElement.setAttribute("data-full-hour", hourVal); 
                                    const modal = new bootstrap.Modal(modalElement);
                                    modal.show();
                                    return false;
                                 }
                           ...
                           function applyTag(cell, label, color = "#000000") { 
            ...
            function updateMonthYearLabel() {
               ...
               label.textContent = `${monthNames[currentDate.getMonth()]} - ${currentDate.getFullYear()}`;
                          ...
                          /* function saveCalendarData(statusValue = 'Submitted') {
   const reportingManagerEmail = document.getElementById("reporting_manager_email");
   if (statusValue === 'Submitted' && (!reportingManagerEmail || reportingManagerEmail.value.trim() === '')) {
      Swal.fire("Required Field", "Please enter Reporting Manager Email Id before submitting.", "error");
      return;
   }
...
function saveCalendarData(statusValue = 'Submitted') {
   const reportingManagerEmail = document.getElementById("reporting_manager_email");
   const corporateEmail = document.getElementById("corporate_email");
   ...
   formData.append('type', 'timesheet');
   formData.append('record', JSON.stringify(records)); // pass array now
   formData.append('user_id', "{{ $userData['id'] ?? '' }}");
   formData.append('client_id', "{{ $consultant->client_id ?? '' }}");
   formData.append('client_name', "{{ $consultant->client_name ?? '' }}");
   formData.append('status', statusValue);
                                       ...
                                       <div class="timeline-item d-flex align-items-start mb-3"
                                             ...
                                             <div>
                                                <div class="d-flex align-items-center mb-1 tl-header">
                                                   ...
                                                   <span style="color: {{ $labelMap[$type]['color'] }}; font-weight: bold;"> {{ $labelMap[$type]['label'] }}
                                                   </span>
                                                </div>
                                                <div class="tl_details">
                                                   <span>{{ $record['applyOnCell'] ?? '--/--/----' }} - </span>
                                                   <span class="badge" style="background-color: #ffe0b3; color: {{ $labelMap[$type]['color'] }}; font-weight: bold;"> {{ $labelMap[$type]['label'] }}
                                                   </span>
